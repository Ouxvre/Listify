...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['task_id'], $_SESSION['user_id'])) {
    ...
    // Ambil data task sebelum dihapus
    ...
    if ($task) {
        // Simpan ke history
        $stmt = $conn->prepare("INSERT INTO task_history (user_id, title, description, deadline, action) VALUES (?, ?, ?, ?, 'deleted')");
        $stmt->bind_param("isss", $user_id, $task['title'], $task['description'], $task['deadline']);
        $stmt->execute();
        $stmt->close();

        // Hapus task dari todos
